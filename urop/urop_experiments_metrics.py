...
# append uncompressed, reconstructed, compressed path to encode command => execute
    if ply_count_raw != ply_count_reconstructed:
        ...
        uncompressed_param = "--uncompressedDataPath=" + os.path.join(uncompressed_path, scope_dataset_name + "_vox10_%04d.ply")
        compressed_param = "--compressedStreamPath=" + os.path.join(compressed_path, experiment_name+".bin")
        reconstructed_param = "--reconstructedDataPath=" + os.path.join(reconstructed_path, scope_dataset_name + "_rec_%04d.ply")

        start_frame_param = '--startFrameNumber=' + start_frame_no[scope_dataset_name]

        command = scope_experiment['encode'] + [start_frame_param, reconstructed_param, compressed_param, uncompressed_param]
...
def execute_decode(scope_curr_experiment_path, scope_experiment, scope_dataset_name):
    ...
    if ply_count_raw != ply_count_decompressed:
        ...
        compressed_param = "--compressedStreamPath=" + os.path.join(compressed_path, experiment_name+".bin")
        decompressed_param = "--reconstructedDataPath=" + os.path.join(decompressed_path, scope_dataset_name + "_dec_%04d.ply")
        start_frame_param = '--startFrameNumber=' + start_frame_no[scope_dataset_name]
